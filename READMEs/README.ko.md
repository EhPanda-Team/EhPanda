...
## 시스템 요구 사항
iOS / iPadOS 버전이 27.0 이상인지 확인해주세요.
...
공식 릴리스는 릴리스 워크플로가 저장소 시크릿에서 가져온 분석 자격 증명을 포함해 빌드되므로, 이 설정을 끄지 않는 한 위에서 설명한 이벤트를 전송합니다. 해당 자격 증명 없이 만든 빌드는 이 설정과 관계없이 아무것도 전송하지 않습니다. 소스에서 직접 빌드한 경우와 포크에서는 자격 증명이 없어 분석이 비활성화됩니다.
