        ...
        importTagTranslator: { url in
            await withCheckedContinuation { continuation in
                guard let data = try? Data(contentsOf: url),
                      let translations = try? JSONDecoder().decode(EhTagTranslationDatabaseResponse.self, from: data)
                    .tagTranslations,
...
extension FileClient {
    static let noop: Self = .init(
        ...
        fetchLogs: { .failure(.notFound) },
        deleteLog: { _ in .failure(.notFound) },
        importTagTranslator: { _ in .failure(.notFound) }
    )
