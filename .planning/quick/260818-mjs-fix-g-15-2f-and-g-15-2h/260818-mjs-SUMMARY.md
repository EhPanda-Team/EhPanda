...
- **PD-5 owner question — the repair-seed materialization is now unreachable from `processDownload`.** With the leaf frozen, the destination equals the record's folder, so `shouldReuseWorkingFolder`'s existence guard and `repairSeed`'s existence guard cannot both fail. `repairSeed`, `materializeRepairSeed` and `RepairSeedContext` were **not** deleted: WR-02 / G-15-13 / G-15-19 pins still own that branch's contract and the locked fix spec did not ask for removal. **Question for the owner: retire the seed materialization in a design round?** Recorded in `15-UAT.md` under G-15-2H as well.
