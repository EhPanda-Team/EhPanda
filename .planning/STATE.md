...
status: executing
stopped_at: Completed 15-73-PLAN.md
last_updated: "2026-08-10T13:39:08.004Z"
last_activity: 2026-08-10
last_activity_desc: "Plan 15-72 executed: verification gap 3 / review WR-03 closed at its root — 15-67 copied the validate route's classify-guard-remove-rescan-blank ORDERING into `authorizedReconciliationScan` and dropped the compensation that ordering obliges. The recover-once-and-log rule now has ONE implementation: `recoveredBlanking` is module-internal and BRACKET-FREE, `blankingPass` lost its bracket, and the validate route supplies ONE span around both of its attempts (DEC-A) — which is exactly the shape the seed route already had, where `prepareWorkingSeed`'s open bracket is what the shared core runs inside. Widening the validate route's bracket is withdrawal-neutral by argument: the credited reading does not move between the attempts and both deltas are downward, so max(d1,0)+max(d2,0) == max(d1+d2,0); it opens BELOW the removal, preserving D-G7-01's no-deletion-inside rule. All THREE post-removal exits are swept with written dispositions on `prepareWorkingSeed`: (1) a rescan that could not enumerate and (2) the post-removal loop's own refusal lines both recover once and PROCEED — decided by one predicate (`claimsAnyPage` over the loop's ANSWER, not its reason, DEC-E) and self-healing because the removed pages are absent from `existingPages` and therefore pending; (3) a thrown manifest write recovers once and PROPAGATES. Exit 3 returns a `Result` out of the bracket rather than throwing through it (DEC-C) because its compensation can SUCCEED and lower the record while a throw would skip the bracket's second reading — G-15-7's masking shape, reintroduced through the very recovery added to close WR-03; the pre-removal steps still throw straight through. `recoveredBlanking` answers with the recovered MANIFEST rather than a Bool (DEC-B) because the seed's record is what the announcement and the fetch filter read. On the basis half, `scanSucceeded` deliberately STAYS sourced from the post-removal rescan (15-67's DEC-F not reverted) and `WorkingSeed.removedPages` is subtracted in BOTH `inheritedPages` branches instead (DEC-D): a removal outranks a rescan in the evidence order, since a page whose file this pass deleted is positively absent by this pass's own ACT and no probe's health can overrule an act. Under-reporting toward re-fetching stays possible (D-G4-01's chosen direction); over-reporting is now structurally impossible. RED banked verbatim: Case A 6 issues incl. a REBUILT coordinator reading `.completed` / `completedPageCount 2` over a destroyed file; Case B1 2 issues (`2 / 3 pages · 1 gallery` against an honest ceiling of 1); Case B2 (healthy rescan) PASSED pre-fix — the boundary evidence. TWO fixture deviations, both recorded: the plan's PERMANENT unwritable manifest could not discriminate in either direction (post-fix the retry fails too, and the 'next converging pass' heals both regimes identically), so Case A stages the TRANSIENT the recover-once path exists for — the double lifts the immutable flag at the SECOND post-removal listing, positionally-independently the recovery's own fresh scan; and the post-removal listing tally is asserted as a FLOOR with the exact pin moved onto the release, because a successful recovery re-indexes the folder and `galleryFolderRecord`'s two rendering resolutions list it again (real count 4, not 2). The validate route's suites are green with ZERO expectation edits — the plan's own falsification condition for a wrong factoring. Grep gates: exactly 1 log call site for the message family, exactly 1 bracket wrapper in PersistenceNormalize.swift, 2 pre-existing bracket calls in ExecutionSupport.swift (none added), 1 blanking-loop declaration. Full FeatureTests 957/0 across 22 targets (166 suites), clean build 0 warnings, SwiftLint --strict 0 violations over all 4 touched files. Commits `1c15cc6f` (RED), `8e263e8d` (GREEN), `414dca25` (docs). **HARD PRECONDITION for the next touch: `DownloadClient+ExecutionSupport.swift` is now 999 of 1000 lines — ONE line of headroom, so it must be SPLIT before anything is added (natural seam: `AuthorizedReconciliation` + `authorizedReconciliationScan` + `inheritedPages` to their own file, mirroring 15-67's DEC-E).** `DownloadFeatureTestHelpers.swift` untouched at 992/1000. Outstanding: UAT test 2 device re-run (system card fraction + cancel parity) now that the announced-basis path changed; and the dispositioned residual where a rescan AND the recovery's rescan both fail, leaving the record legitimately claiming the removed page with the `error` log as its trail. Gaps 1/4/5 remain with plans 15-73..15-77. Prior context: Plan 15-69 executed: verification gap 4 closed — WR-04 and WR-05, the two caller-side consequences of the CR-04 narrowing. `retryPages`' inadmissible-selection exit now returns `.fileOperationFailed(String(localized: .downloadStoreInvalidPageSelection))` while the absent-gallery and absent-folder exits KEEP `.notFound` (DEC-A: absence is what both of them mean), so no localized message conflates \"this download is gone\" with \"the pages you named are outside this gallery\". `normalizeFetchedPayload` is now `throws(AppError)`: a non-nil selection the freshly fetched page count empties raises the named page-selection-outdated error (DEC-B), which propagates through `fetchNormalizeAndDownload` to `processDownload`'s existing catch and settles via `persistFailure` — confirmed line by line to precede `performDownload` entirely, so no working seed, announcement, cover download or `finalizeBatchResult` whole-manifest measurement runs, and the gallery no longer settles into a persistent `.error` record for work nobody requested. The inspector's `retryPagesDone(.failure)` arm sets `state.toast` through a private `AppError.retryFailureToast` mapping (payload for `.fileOperationFailed`, `alertText` otherwise, `localizedDescription` as the never-empty fallback) beside the existing `toastConfig`, wiring the toast surface the view already presents. Two module-local catalog keys added in all six locales, both plain strings (no numeric specifier). Three-state consumer sweep enumerated from source with dispositions for `shouldSchedule`'s gate, the announcement gate, `downloadCoverImage`, `finalizeBatchResult`/`missingFinalizedPageIndices` and the `rawPageSelection` bridge; `pendingPageIndices` keeps its present-empty branch as defence in depth. RED banked 10 verbatim issues; the full-suite gate caught an un-swept assertion (`DownloadRetryUpdateFallbackTests.swift:140` pinned `.notFound` for the update record, which is refused by the SAME exit). Full FeatureTests 950/0 across 22 targets (downloads 431 in 72 suites), clean build 0 warnings, SwiftLint --strict 0 violations in 120 files. Commits `34984354` (RED), `1d966aee` (GREEN). OPEN and independent: CR-01 (unbracketed `advanceQueueIntentGeneration`, gap 1) and gap 2's read-path half; IN-02 (two localized-key spellings in `DownloadStore+Operations.swift`) unrouted."
progress:
  total_phases: 16
  completed_phases: 13
  total_plans: 248
...
Plan: 73 of 77
Status: Executing — round-20 gap + follow-up plans 15-70..15-77; 15-70..15-72 done, 15-73..15-77 remain
Last activity: 2026-08-10 — Plan 15-72 executed: verification gap 3 / review WR-03 closed at its root — 15-67 copied the validate route's classify-guard-remove-rescan-blank ORDERING into `authorizedReconciliationScan` and dropped the compensation that ordering obliges. The recover-once-and-log rule now has ONE implementation: `recoveredBlanking` is module-internal and BRACKET-FREE, `blankingPass` lost its bracket, and the validate route supplies ONE span around both of its attempts (DEC-A) — which is exactly the shape the seed route already had, where `prepareWorkingSeed`'s open bracket is what the shared core runs inside. Widening the validate route's bracket is withdrawal-neutral by argument: the credited reading does not move between the attempts and both deltas are downward, so max(d1,0)+max(d2,0) == max(d1+d2,0); it opens BELOW the removal, preserving D-G7-01's no-deletion-inside rule. All THREE post-removal exits are swept with written dispositions on `prepareWorkingSeed`: (1) a rescan that could not enumerate and (2) the post-removal loop's own refusal lines both recover once and PROCEED — decided by one predicate (`claimsAnyPage` over the loop's ANSWER, not its reason, DEC-E) and self-healing because the removed pages are absent from `existingPages` and therefore pending; (3) a thrown manifest write recovers once and PROPAGATES. Exit 3 returns a `Result` out of the bracket rather than throwing through it (DEC-C) because its compensation can SUCCEED and lower the record while a throw would skip the bracket's second reading — G-15-7's masking shape, reintroduced through the very recovery added to close WR-03; the pre-removal steps still throw straight through. `recoveredBlanking` answers with the recovered MANIFEST rather than a Bool (DEC-B) because the seed's record is what the announcement and the fetch filter read. On the basis half, `scanSucceeded` deliberately STAYS sourced from the post-removal rescan (15-67's DEC-F not reverted) and `WorkingSeed.removedPages` is subtracted in BOTH `inheritedPages` branches instead (DEC-D): a removal outranks a rescan in the evidence order, since a page whose file this pass deleted is positively absent by this pass's own ACT and no probe's health can overrule an act. Under-reporting toward re-fetching stays possible (D-G4-01's chosen direction); over-reporting is now structurally impossible. RED banked verbatim: Case A 6 issues incl. a REBUILT coordinator reading `.completed` / `completedPageCount 2` over a destroyed file; Case B1 2 issues (`2 / 3 pages · 1 gallery` against an honest ceiling of 1); Case B2 (healthy rescan) PASSED pre-fix — the boundary evidence. TWO fixture deviations, both recorded: the plan's PERMANENT unwritable manifest could not discriminate in either direction (post-fix the retry fails too, and the 'next converging pass' heals both regimes identically), so Case A stages the TRANSIENT the recover-once path exists for — the double lifts the immutable flag at the SECOND post-removal listing, positionally-independently the recovery's own fresh scan; and the post-removal listing tally is asserted as a FLOOR with the exact pin moved onto the release, because a successful recovery re-indexes the folder and `galleryFolderRecord`'s two rendering resolutions list it again (real count 4, not 2). The validate route's suites are green with ZERO expectation edits — the plan's own falsification condition for a wrong factoring. Grep gates: exactly 1 log call site for the message family, exactly 1 bracket wrapper in PersistenceNormalize.swift, 2 pre-existing bracket calls in ExecutionSupport.swift (none added), 1 blanking-loop declaration. Full FeatureTests 957/0 across 22 targets (166 suites), clean build 0 warnings, SwiftLint --strict 0 violations over all 4 touched files. Commits `1c15cc6f` (RED), `8e263e8d` (GREEN), `414dca25` (docs). **HARD PRECONDITION for the next touch: `DownloadClient+ExecutionSupport.swift` is now 999 of 1000 lines — ONE line of headroom, so it must be SPLIT before anything is added (natural seam: `AuthorizedReconciliation` + `authorizedReconciliationScan` + `inheritedPages` to their own file, mirroring 15-67's DEC-E).** `DownloadFeatureTestHelpers.swift` untouched at 992/1000. Outstanding: UAT test 2 device re-run (system card fraction + cancel parity) now that the announced-basis path changed; and the dispositioned residual where a rescan AND the recovery's rescan both fail, leaving the record legitimately claiming the removed page with the `error` log as its trail. Gaps 1/4/5 remain with plans 15-73..15-77. Prior context: Plan 15-69 executed: verification gap 4 closed — WR-04 and WR-05, the two caller-side consequences of the CR-04 narrowing. `retryPages`' inadmissible-selection exit now returns `.fileOperationFailed(String(localized: .downloadStoreInvalidPageSelection))` while the absent-gallery and absent-folder exits KEEP `.notFound` (DEC-A: absence is what both of them mean), so no localized message conflates "this download is gone" with "the pages you named are outside this gallery". `normalizeFetchedPayload` is now `throws(AppError)`: a non-nil selection the freshly fetched page count empties raises the named page-selection-outdated error (DEC-B), which propagates through `fetchNormalizeAndDownload` to `processDownload`'s existing catch and settles via `persistFailure` — confirmed line by line to precede `performDownload` entirely, so no working seed, announcement, cover download or `finalizeBatchResult` whole-manifest measurement runs, and the gallery no longer settles into a persistent `.error` record for work nobody requested. The inspector's `retryPagesDone(.failure)` arm sets `state.toast` through a private `AppError.retryFailureToast` mapping (payload for `.fileOperationFailed`, `alertText` otherwise, `localizedDescription` as the never-empty fallback) beside the existing `toastConfig`, wiring the toast surface the view already presents. Two module-local catalog keys added in all six locales, both plain strings (no numeric specifier). Three-state consumer sweep enumerated from source with dispositions for `shouldSchedule`'s gate, the announcement gate, `downloadCoverImage`, `finalizeBatchResult`/`missingFinalizedPageIndices` and the `rawPageSelection` bridge; `pendingPageIndices` keeps its present-empty branch as defence in depth. RED banked 10 verbatim issues; the full-suite gate caught an un-swept assertion (`DownloadRetryUpdateFallbackTests.swift:140` pinned `.notFound` for the update record, which is refused by the SAME exit). Full FeatureTests 950/0 across 22 targets (downloads 431 in 72 suites), clean build 0 warnings, SwiftLint --strict 0 violations in 120 files. Commits `34984354` (RED), `1d966aee` (GREEN). OPEN and independent: CR-01 (unbracketed `advanceQueueIntentGeneration`, gap 1) and gap 2's read-path half; IN-02 (two localized-key spellings in `DownloadStore+Operations.swift`) unrouted.
...
Last session: 2026-08-10T13:38:57.589Z
Stopped at: Completed 15-73-PLAN.md
Resume file: None
