...
status: executing
stopped_at: Completed 15-76-PLAN.md
last_updated: "2026-08-10T15:21:46.430Z"
last_activity: 2026-08-11
last_activity_desc: "Plan 15-76 executed: the owner's first Deferred Follow-Up (15-UAT.md test 6) CLOSED in code. `Defaults.FilePath.logs` is now "Logs" and a one-time launch migration (`LogsDirectoryMigration`) moves a pre-rename install's directory onto it. The real finding is that the test machine is a THIRD filesystem regime neither the plan nor the obvious probe accounts for: the iOS simulator reports case-SENSITIVE lookups (`fileExists(".../Logs")` -> NO while `logs` is stored) over a case-INSENSITIVE macOS APFS backing store (`createDirectory(".../Logs")` -> EIO), whereas the same path from the Mac answers the opposite (fileExists True, mkdir EEXIST). The both-exist test gate had been written as a lookup probe, so it answered "case-sensitive" here and ran a case whose fixture is impossible to build -- caught by watching the first RED run die in its own setup (deviation 1). DEC-A: the gate now STAGES the precondition (create `logs` and `Logs`, require two listing entries). DEC-B: regime classification was extracted as a pure total function of (storedNames, currentSpellingResolves), so all four regimes -- including the merge this host cannot stage -- are pinned by test on any volume; `mergeContents` takes arbitrary URLs so the merge APPLICATION also runs everywhere. Only the composition of the two halves is host-gated (coverage D8, skipped here). The case-insensitive detection is DERIVED, not assumed: a name that resolves without being stored must be an alias for one that is. DEC-D: the device regime keeps a single atomic moveItem (no crash window); staging is used only where atomicity is unavailable anyway. DEC-E: a merge skips a colliding name and keeps the DESTINATION copy, because run-log names embed day+time+count so a shared name is the same run and the destination copy may be the one being appended to; nothing is ever overwritten and only an emptied source directory is removed. DEC-C: the legacy name is a FROZEN private literal, not `logs.lowercased()` -- deriving it would let a future rename silently redefine the migration's SOURCE and strand the directory this plan just created. DEC-F: the plan's acceptance criterion (no `"logs"` literal anywhere) contradicts its own required artifact and was replaced by one carrying the intent -- exactly one private lowercase literal survives, used only as a move source (deviation 2). Rule 2 addition: a user-created regular FILE named `logs` (File Sharing writes into Documents) is now left alone, since moving it onto `Logs` would put a file where `appendToRunFile` must create a directory and break logging outright (deviation 3). Race tolerance restated honestly per regime: absorbed in-place on case-insensitive volumes, in-launch for a write arriving mid-staging, and at the NEXT launch for the atomic device regime -- no strict ordering against launch logging is claimed. Consumer census re-derived (case-insensitively, incl. Info.plist, entitlements, pbxproj and interpolated paths): the plan's table was CORRECT -- 6 consumers all deriving from `FileUtil.logsDirectoryURL`, 1 change site, no other path literal. Gates: 980 tests / 22 targets / 0 failures / 1 correctly skipped (963 baseline + 17 new); fresh-derived-data build 0 warnings 0 errors; SwiftLint --strict 0 violations over 4 files; Package.swift untouched. RED was watched failing at 0f45c40f (17 tests, 12 failing, 22 issues). Open, non-blocking, both riding the next device UAT pass: coverage D8 (both-exist merge pinned in halves, not observed joined) and D9 (`openFileApp`'s shareddocuments:// deep link derived to open `Logs`, never observed). The second owner follow-up (DownloadsView swipe-action offset during the deletion alert) remains open."
progress:
  total_phases: 16
  completed_phases: 13
  total_plans: 248
...
- [Phase ?]: DEC-A (15-68): the user-folder boundary is a closure-taking function (mutatingConfinedUserFolder), so a mutation has nowhere to go but through the confined resolver
- [Phase ?]: DEC-B (15-68): userFolderURL(name:) is deleted rather than demoted — unreachability enforced by the symbol's absence, not by an access level the same module could reach
- [Phase ?]: DEC-C (15-68): enqueue's parent-folder create is removed as redundant (writeInitialManifest already creates it with intermediates) rather than confined, which would have made a scan-derived non-normalized folder un-enqueueable
...
- [Phase ?]: DEC-A (15-73): the pause failure arm reuses the retry branch's mapping verbatim (toast then reload), renamed actionFailureToast for its two families
- [Phase ?]: DEC-B (15-73): the disposition sweep is stated ONCE per reducer as a type-level enumeration, with per-case one-line anchors, so a new outcome-carrying action with no disposition contradicts a doc
- [Phase ?]: DEC-C (15-73): DownloadsReducer.toggleDownloadPauseDone is recorded as SILENT-AND-WEAKEST rather than deliberately silent; reporting it needs a toast surface the list does not own, so it is logged as an open item instead of given a justification it does not have
- [Phase ?]: 15-74: the counted-basis bracket's SIBLINGS-only rule is a convention with a runtime depth-counter detector (reportIssue, never a trap), not a type-system guarantee — 15-65-SUMMARY corrected in place
- [Phase ?]: 15-74: IN-01's one-second detector bound declined on recorded evidence (13.2s wall under contention at that exact case); ten-second default kept and the derivation written at the call site — owner ratification requested
...
- [Phase ?]: DEC-A: the both-exist test gate stages two case variants rather than probing fileExists — the iOS simulator reports case-SENSITIVE lookups over a case-INSENSITIVE backing store, a third regime the plan does not describe
- [Phase ?]: DEC-B: logs-migration regime classification is a pure total function, so the merge regime is pinned by test on a host whose volume cannot represent it
- [Phase ?]: DEC-C: the legacy logs directory name is a frozen literal, not derived from the current constant, so a future rename cannot silently redefine the migration's source
- [Phase ?]: DEC-D: the device regime keeps a single atomic moveItem; staging is used only where atomicity is unavailable anyway
- [Phase ?]: DEC-E: a logs merge skips a colliding name and keeps the destination copy, because run-log names embed day+time+count so a shared name is the same run and the destination copy may be live
