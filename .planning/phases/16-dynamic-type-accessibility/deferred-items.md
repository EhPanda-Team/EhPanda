...
- `AppPackage/Sources/DetailFeature` `LinkColoredText` (comment link runs): `16-CONTRAST-AUDIT.md § Decisions › D28 = ok` and its D-25 table attribute the `.underlineStyle(.single)` link carrier to plan 16-22, but plan 16-22's tasks and `files_modified` do not include it (its files are the Activity Logs and Laboratory views and the audit), and the same slot's "palette-rendered" level glyphs were superseded by the plan's `keep .foregroundStyle(level.color)`. Not built here (plan fidelity); the underline has no plan and needs one — 16-23 (which already owns the `comment-link` colour change in the same file) is the natural home. Recorded in the audit's `### 16-22 result (DWC)` as still open.
