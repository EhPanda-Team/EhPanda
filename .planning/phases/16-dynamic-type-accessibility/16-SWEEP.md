...
| 39 | #8 Favorites | iPhone portrait AX5 | The native Search drawer is a blank capsule at AX5, with its magnifier and placeholder missing; the `.large` reference retains both. The title and gallery metadata remain complete and the tail remains reachable. | accepted — owner identifies this as an Apple native-search bug; no app fix requested; remains one measured finding and does not add a pass cell |
...
| VO-3 | F2 | 1.8 | Reproduces. Search › More › Filters › `Cancel` does not return focus to `More`: from the root it lands on the `Search` heading; from the results it lands on the error toast. Candidate `visiblebodyMore` is carried to 16-26 sign-off; the original target remains unmet and is not accepted. | `…/transcripts/vo3-filters.txt`, `…/transcripts/f2-filters.txt` | owner (D-22): carried to 16-26 sign-off; cause identified 2026-09-19: SwiftUI bridges a `Label`-labeled toolbar control to a native bar button item, which drops the focus binding. Owner ruling 2026-09-20: Apple-bug handling, do not fix locally; the evaluated `Image`-label workaround was not adopted and focus stays at the platform default (`16-FOCUS-INVESTIGATION.md § Root-cause round (2026-09-19)`) | — |
...
| W-8 | F3 | 1.3 | Once in the Task 2 walk, about 1 s after focus reached the uploader with no key pressed, VoiceOver logged `Screen Changed` and reset focus to `More`; two reproduction attempts (45 s idle each, two launch routes) logged nothing. The listening recordings (R13) reproduced it in 2 of 4 Detail launches: in L-2 focus reached the uploader at 20.09 s, `Screen Changed` at 20.86 s, focus `More` at 21.64 s; in L-5 at 17.88 s, 18.65 s and 19.43 s, so the reset follows about 0.77 s after focus reaches the uploader | `…/transcripts/f3-detail-walk.txt`, `…/transcripts/w-detail-screenchange-2.txt`, `…/transcripts/w-detail-screenchange-3.txt`, `…/listen/audio/run1/L-2.vot.log`, `…/listen/audio/run1/L-2.report.txt`, `…/listen/audio/run1/L-5.vot.log`, `…/listen/audio/run1/L-5.report.txt` | fix (was `deferred`; reproduced by the recording and moved, R13); characterized 2026-09-19 as system-side: SpringBoard posts one deferred `Screen Changed` per app activation at a nondeterministic time, which explains 2 of 4 launches; no app-side lever found, the activation-order conclusion is superseded, owner disposition outstanding (`16-FOCUS-INVESTIGATION.md § Root-cause round (2026-09-19)`) | walk 2: reproduced uploader `02:49:37.524` → Screen Changed `02:49:38.302/.318` → More `02:49:39.095`; audio `28.01 s` → `29.60 s`, unresolved ($HOME/Library/Caches/ehpanda-phase16/round2/walkthrough/listen/audio/task7-final-20260917-0248/L-2.vot.log; $HOME/Library/Caches/ehpanda-phase16/round2/walkthrough/listen/audio/task7-final-20260917-0248/L-2.report.txt) |
...
| W-33 | listening (L-6) | 1.2 (speech) | A comment's time is heard as "four thirty two" with no "PM". The date and time are app-authored (a formatted value); the spoken string carries U+202F (narrow no-break space) before "PM" (`4:32\u202fPM` in the `vot` log), but that this character is why "PM" is dropped is unproven | `…/listen/audio/run1/L-6.vot.log`, `…/listen/audio/run1/L-6.report.txt`, `…/listen/audio/clips/06-comment-time.m4a` | fix — owner phonetic verification passed 2026-09-18 | walk 2: corrected clip `$HOME/Library/Caches/ehpanda-phase16/round2/walkthrough/w33/rootcause-20260918/audio/root-final-0139-date-pm-corrected.wav`, source 17.000–21.500 s, SHA-256 `d212aecb148a09fd8ed04b064f9acc2aea9ec57e601a5582f6d8d6c63fc3ec55`; prior capital-letter clip remains historical |
...
| W-35 | owner report (Detail) | 1.7 | After Detail opens, VoiceOver focus goes to the `More` button first, not to the gallery title. Owner: "進入 detail 後的焦點不在標題而是先到 more button"; target stated by the owner: the title | `…/transcripts/f3-detail-walk.txt`, `…/listen/audio/run1/L-2.vot.log` | fix (platform rule identified 2026-09-19: on `Screen Changed` VoiceOver takes the app's first element in traversal order, and the navigation bar precedes the content; no synchronous public hook exists; owner decision among three options outstanding; `16-FOCUS-INVESTIGATION.md § Root-cause round (2026-09-19)`) | walk 2: first Detail focus still reaches More and remains below the title target; same L-2 evidence ($HOME/Library/Caches/ehpanda-phase16/round2/walkthrough/listen/audio/task7-final-20260917-0248/L-2.vot.log; $HOME/Library/Caches/ehpanda-phase16/round2/walkthrough/listen/audio/task7-final-20260917-0248/L-2.report.txt) |
...
The owner-routed records are `VO-3` (Filters dismissal focus is carried to 16-26 sign-off) and `W-22` (owner decision to show the signed-out Watched title). W-34 is a completed fix with owner phonetic verification passed 2026-09-17. Explicit carried item is `VO-3`; W-38 awaits an owner operation demonstration for the Reader page-52/indicator mismatch and is not a newly reproduced or accepted case. The deferred records are `W-21` (ContentUnavailableView symbol) and `W-24` (Search keyword Delete glyph contrast). The 15 unreached sites are retained in the Swept sites table with their concrete reasons; they are not promoted to pass by this index.
...
Tasks 3–4 owner sign-off remains pending. W-33 is in revision for the requested ordinary “P M” pronunciation; W-34 is a completed fix with owner phonetic verification passed for the MiB pronunciation; no 16-26
...
The first line closes W-34 only for the MiB pronunciation. The second line leaves W-33 uncertain because the PM phrase was not accepted. A short W-33 follow-up clip is available at `$HOME/Library/Caches/ehpanda-phase16/round2/walkthrough/listen/audio/task7-final-20260917-0248/clips/w33-time-short.m4a`, with source interval 359.350–363.300 s (3.950 s, normal speed, unaltered) and provenance at `$HOME/Library/Caches/ehpanda-phase16/round2/walkthrough/listen/audio/task7-final-20260917-0248/clips/w33-time-short-provenance.md`; its ASR is only machine positioning and remains unaccepted. Separately, the user accepted only the W-31 scope of the empty uploader button capsule under Button Shapes: 「收尾記錄裡面那些問題，只接受空白按鈕膠囊，其它問題需要你詳細解釋我判斷」. This W-31 scope statement does not broaden the separate #39 Apple native-search ruling or accept any other blank control. W-8, W-35, VO-3, W-13 Comments, and W-38 remain unresolved; Voice Control spoken actuation and VoiceOver double-tap activation remain unmeasured limitations.

### Owner decision follow-up (recorded 2026-09-17T16:01:34Z)

The covered repository documentation HEAD is `812c85c7aa1342606f401e7de6e5d1924e5839ca`; the measured source remains `b01add4c11b1f9c355ac8f2e055ed8b24fe8146c`. This records the latest user decisions and requests without claiming phase approval, A11Y-02 completion, or a new runtime measurement.

- D-25 finding #39 (Favorites AX5 blank native Search capsule) is accepted as an Apple native-search bug with no app fix requested. The nine-cell measurement remains eight pass cells plus this one accepted finding.
- W-8, W-35, VO-3, and W-13 require root-cause investigation. They are not accepted and are not classified as system limits.
- W-38 awaits an owner operation demonstration before judgment; it is not accepted or treated as a new reproduced case.
- Historical W-33 status: the ordinary `P M` pronunciation was in revision while the replacement candidate and audio judgement were pending. This entry is superseded by the current owner-verified result below; the earlier feedback remains historical. No UGC wording is reproduced here.
...
Voice Control spoken actuation remains unmeasured. Native VoiceOver double-tap has bounded evidence for the W-38 reader-panel reveal; activation across the other walkthrough flows remains unmeasured. These coverage limits are not owner decisions. The approved natural colour acceptance applies to W-22 only and is not phase sign-off.
...
### Current iOS 27 closing-gate continuation (2026-09-23)

Current verification uses Xcode 27.0 at `/Applications/Xcode-27.0.0.app/Contents/Developer`, with the task-owned iPhone Air (`8E3EA338-4F93-40A7-BE4F-1F6E7C855F32`, iOS 27.0) and iPad Pro 11-inch (M5) (`55C7BED8-A301-4C4B-94F7-1959B45EBE27`, iPadOS 27.0). The historical Xcode 26.6/iOS 26.5 results above remain historical. The ignored local `workflow.test_command` now names this task-owned iPhone explicitly. The original D-25 scope and its nine cells remain unchanged.

Evidence root: `/tmp/ehpanda-phase16-close-20260923/`. The initial source snapshot is repository HEAD `a487965b3411f71cdb37b570771e86107a7b3136` plus the pre-existing `EhPanda.xcodeproj/project.pbxproj` edit, captured in `preexisting-project.patch`. `source-before.json` hashes all 809 tracked source/configuration files in the measured scope; no source changed while the initial tests executed. This is an exact working-tree snapshot, not a clean-tree claim.

| Gate | Result | Evidence within the evidence root |
|---|---|---|
| FeatureTests, iPhone | 1066 total: 1055 passed, 11 expected failures, 0 failed, 0 skipped, Repetition 0; result interval 125.250 s | `featuretests-iphone.{log,xcresult}`, `featuretests-iphone-{summary,tests}.json` |
| Initial UITests, iPhone | Failed: 56 unique tests, 53 passed, 1 failed, 2 expected iPad-only skips; 58 executions and 3 Repetition nodes. Test execution 1269.681 s; result interval 1901.032 s includes post-failure diagnostic collection. Exit 65 | `uitests-iphone.{log,xcresult}`, `uitests-iphone-{summary,tests}.json` |
| Corrected UITests, iPhone | Passed: 56 total/executions, 54 passed, 2 expected iPad-only skips, 0 failed, Repetition 0; result interval 1103.511 s, console 1106.168 s; exit 0 | `uitests-iphone-after-route-fix.{log,xcresult}`, matching `-summary.json` / `-tests.json` |
| UITests, iPad | Failed first-try gate: 56 unique tests, 55 eventual passes, 1 final failure, 0 skips; 59 executions and 5 Repetition nodes; result interval 1416.136 s, exit 65 | `uitests-ipad.{log,xcresult}`, matching `-summary.json` / `-tests.json` |

The sole failing test in the initial iPhone run was `AccessibilityAuditUITests.testQuickSearchSheetAudit`. All three attempts stopped at the obsolete `More` lookup before reaching Quick Search. Search root intentionally exposes the direct `Quick Search` and `Filters` toolbar buttons after `ee84c53e` / `6394fd61`; the pushed Search view has a different toolbar. The orchestrator approved only the test-route correction recorded in `16-26-PLAN.md`: remove the obsolete menu step and retain the direct button lookup, sheet-title assertion, accessibility audits and dismissal. No production code, audit exclusion, timeout, test repetition policy or reader index logic changed. The failed result is retained; it is not a first-try pass.

All five ordinary-touch reader tests passed on the initial iPhone run, including scrolling, slider, tap zones, autoplay and resume. The separate bounded iPad diagnostic reached placeholder 3 / panel 3 after one ordinary swipe and traced the existing update call; its limitations and evidence are in `16-W38-ROOT-CAUSE.md § Separate ordinary-touch diagnostic`. That diagnostic does not clear the earlier intermittent iPad automated failure.

Xcode's post-failure delay was independently sampled as `collectSimulatorDiagnostics` / `simCtlDiagnose`, with a 600-second collection timeout; the original process was allowed to exit naturally. The test-only patch was applied after every original test had finished, while diagnostics were being collected. `route-fix-provenance.json` records that only `EhPandaUITests/AccessibilityAuditUITests.swift` changed and the pre-existing project edit was preserved. The changed file passed strict no-cache SwiftLint with zero violations.

The prior-phase regression inventory inspected all 12 earlier verification records: 27 referenced test basenames resolve to 33 current files (29 FeatureTests files and four UI-test files). `AppRouteReducerTests.swift` was renamed to `PresentationFeatureTests.swift` in `5fe05eed`, retaining the injected UserDefaults checks; the historical `SystemNotificationExtTests` target maps to `SystemNotificationTests`. No reference remains unmapped. Evidence is `regression-files.json` and `regression-coverage.json`; the full plans cover these tests without a separate redundant regression run.

The test-route correction is committed as `9ea2f359` (`test(16): follow direct Quick Search toolbar`). FeatureTests remain current because the only tested-source delta is that UI-test navigation step; no FeatureTests target or production source changed. The corrected-route focused run passed 1/1, with zero failures, skips or Repetition nodes and exit 0 (`quick-search-route-iphone.{log,xcresult}`, matching summary/tests JSON). The focused run's auxiliary `simctl diagnose` collection also stalled after a passed test; root interrupted only that task-owned collector after preserving the original ten-minute diagnostic attempt. Xcode and the test runner were not interrupted. The remaining full UI runs use the documented `-collect-test-diagnostics never` option, omitting only verbose system diagnostics while retaining test results, failure reporting, screenshots and the unchanged retry policy. `xb27-original.sh` preserves the original wrapper; `xb27.sh` records the subsequent option.

The source fingerprint remained unchanged throughout the corrected full iPhone run. Xcode had already shut down its task-owned destination at completion; the redundant cleanup command returned an already-Shutdown error after the verified pass. The temporary runner now treats that state as successful cleanup and resumed only the iPad gate; no iPhone test was rerun.

The full iPad run reproduced two distinct reader-test failures. Autoplay's first attempt found zero `OverflowBarButtonItem` buttons during its final direction-reset setup; its retry passed. Ordinary vertical scrolling reported panel page 2 / measured page 3 on all three attempts. The latter is the sole final failed test, but both tests fail the first-try requirement. All other iPad tests passed, including the final Safari share handoff. Source hashes remained unchanged through the full gates (`source-after-full-gates.json`).

The exported iPad recording shows that the ordinary swipe stops near a page boundary: page 3's number sits partly behind the bottom controls while the panel still says 2. The probe currently measures the nearest numeric text, although the placeholder's number sits above its centered number-and-Reload stack. This suggests a measurement-boundary problem but does not establish one without frame evidence. A focused diagnostic adds only failure attachments (screenshot and one snapshot's numeric/Reload frames), preserving every assertion, gesture, timeout and product source. Its result is diagnostic evidence, not a replacement closing pass. Original full-gate evidence remains intact under `ipad-reader-attachments/`.

Standalone strict, no-cache SwiftLint passed with `Found 0 violations, 0 serious in 586 files` (`swiftlint.log`) before the diagnostic attachment change; that changed helper separately passed strict lint. The six custom rules remain at error severity and the phase-range added-media count is zero (`static-checks.json`). Current status: the corrected full iPhone gate passed; the iPad first-try gate is red and diagnosis is in progress. Final phase sign-off remains pending; Phase 16/A11Y-02 remains open.


#### Reader-helper diagnosis and correction

The diagnostic-only bundle `reader-geometry-ipad-2.xcresult` failed its first two attempts and passed its third; it is not a closing pass. Its two captured failure snapshots agree exactly: viewport center `(417, 605)`; page 2 number center `(417, -15)` and number/Reload union center `(417, 20)`; page 3 number center `(417, 1195)` and union center `(417, 1230)`. The full-height page boundary is y=625, so page 2 still covers the viewport center. The number-only probe falsely chose 3. Evidence and arithmetic are retained in `reader-geometry-attachments/` and `reader-geometry-comparison.json`.

The corrected probe pairs each numeric text with its nearest Reload button by geometry and compares their centered unions, all from one snapshot. Horizontal accessibility-tree order differs from vertical order, so a preliminary sibling-order implementation was rejected after `reader-center-ipad.xcresult` exposed its incorrect pairing; the captured horizontal frame evidence showed page 2 and panel 2 already agreed. The final geometric pairing passed `testScrollingKeepsTheIndicatorOnThePageShown` in both directions on its first attempt (`reader-center-paired-ipad.{log,xcresult}`, 1 pass, zero failures/skips/Repetition nodes). The test now drags vertically from 80% to 20% of the viewport to cross the half-page boundary, retaining its original forward-advancement assertion. No reader index or other production source changed.
