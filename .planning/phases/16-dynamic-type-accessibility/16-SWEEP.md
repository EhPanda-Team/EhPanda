...
| 29 | #39 | iPhone portrait XXL / AX3 / AX5; iPhone landscape XXL / AX3 / AX5 | The Filters sheet's category grid is a fixed three-column grid of `CategoryCell`s (`AppComponents/CategoryView.swift:87`, `lineLimit(1)`) whose cells keep their width while their names grow, so the names are eaten from the right. At the default size all nine read in full. At XXL two of them are already ellipsised; at AX3 eight of the nine are cut to three or four letters; at AX5 every cell is one or two letters plus an ellipsis, and several become mutually indistinguishable — two cells reading a single capital plus an ellipsis in the same colour family are the only thing telling a user which category a tap will toggle. The colour is the cell's other identifier, but colour alone is not a name, and the grid is the sheet's primary control. Landscape does not help: the grid simply lays out six per row instead of three, keeps the same per-cell width, and shows the identical progression — two cut at XXL, eight at AX3, all nine at AX5. | open |
...
| 31 | #42 | iPhone portrait XXL / AX3 / AX5; iPhone landscape AX3 / AX5 | The error toast's title stays complete, but its subtitle is hard-capped to one line (`SystemNotification/ToastMessageView.swift:70`) and progressively loses the unsupported-link explanation as Dynamic Type grows. The complete sentence is visible in landscape XXL, while portrait XXL already ends after `recognized…`, portrait AX3 after `was…`, and portrait AX5 after `link…`; landscape AX3 and AX5 also ellipsise. The detail sheet reached by tapping the toast reflows and scrolls through the complete Description, Suggested Solution, Context and Environment sections at every sampled size, so this finding is confined to the toast's immediate message. | open |
