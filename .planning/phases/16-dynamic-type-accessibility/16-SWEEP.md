...
| Search root (#W-22) | v3b changes the `SuggestionsPanel` intrinsic-height floor, title identity and search layout; this is a rendered-layout change and requires a fresh D-25 re-sweep | XXL/.large baseline, AX3, AX5; iPhone portrait and iPad portrait; S1/S2/S3, plus S3 AX5 landscape on both devices | **open / pending commit** — evidence and frames are recorded below; root review remains required for Favorites/Watched pixel FAIL/PENDING and any unmatched cell |
...
VO-3 results remain negative for native-menu A, label B and native-menu-combine C. B/C environment and UIKit checks passed, and More/Filters keyboard pre-focus succeeded; Cancel → Search-back lacks a focus binding change and remains under investigation. The approved six-line `SearchView.swift` trial was withdrawn to HEAD and has no validated fix. W-35's two-scope negative trial was also withdrawn to HEAD and remains open without a validated fix. W-22's accepted BEFORE set covers 18 Search cells, 8 Watched/Favorites cells, both bottom/live states and landscape; its production/AFTER run remains in progress and its full cell table remains owned by W-22.
...
The F body native button's v1/v2 diagnostics reached the actual `Filters` control and its spoken label through the toolbar path; the focus binding remained `false` in the logs, but that synchronization result is not itself a runtime failure. This positive-control result does not establish a root cause or close the remaining W-13 focus design.
...
The W-22 intrinsic-height and W-10 toast sort-priority edits remain uncommitted source changes. The W-13 Detail changes in `DetailView.swift` and `DetailView+HeaderSection.swift` also remain uncommitted; all three source areas are covered by the combined gate above. The W-22 S2 AX5 v2-before-fixedSize measurement was `125` → `63`; the v3 fixedSize measurement is still pending. The F-body positive control reached the actual `Filters` control and its spoken label in v1/v2, while the toolbar focus diagnostic remained negative; the binding's synchronized `false` value is not itself a runtime failure.
...
The following rows expand every Search cell as required by W-22. Paths are written relative to `$HOME/Library/Caches/ehpanda-phase16/round2/walkthrough/vo22`; `pending` and `unknown` preserve gaps in the captured UI/readback rather than inferring a value.
...
|---|---|---|---|---|---|
| iphone S1 large | `iphone-large-S1.txt`, `iphone-large-S1.png`; `iphone-content-size-final-readback.log` = `large` | `16,64 94x41` / `pending` / `pending` | `after-verified-v3b/iphone-cold-S1-large-ui.txt`, `after-verified-v3b/iphone-cold-S1-large.png`; `iphone-cold-S1-large-readback.log` = `large` | `16,64 112x41` / `16,117 370x44` / `pending` | PENDING/INSPECT |
| iphone S1 AX3 | `iphone-true-AX3-S1.txt`, `iphone-true-AX3-S1.png`; `iphone-true-AX3-readback.log` = `accessibility-extra-large` | `pending` / `16,117 370x96` / `pending` | `after-verified-v3b/iphone-cold-S1-accessibility-extra-large-ui.txt`, `after-verified-v3b/iphone-cold-S1-accessibility-extra-large.png`; `iphone-cold-S1-accessibility-extra-large-readback.log` = `accessibility-extra-large` | `16,61 124x46` / `16,117 370x96` / `pending` | PENDING/INSPECT |
| iphone S1 AX5 | `iphone-true-AX5-S1.txt`, `iphone-true-AX5-S1.png`; `iphone-true-AX5-readback.log` = `accessibility-extra-extra-extra-large` | `pending` / `16,117 370x124` / `pending` | `after-verified-v3b/iphone-cold-S1-accessibility-extra-extra-extra-large-ui.txt`, `after-verified-v3b/iphone-cold-S1-accessibility-extra-extra-extra-large.png`; `iphone-cold-S1-accessibility-extra-extra-extra-large-readback.log` = `accessibility-extra-extra-extra-large` | `16,61 124x46` / `16,117 370x124` / `pending` | PENDING/INSPECT |
| iphone S2 large | `iphone-large-S2.txt`, `iphone-large-S2.png`; `iphone-content-size-final-readback.log` = `large` | `16,64 94x41` / `pending` / `pending` | `after-verified-v3b/iphone-cold-S2-large-ui.txt`, `after-verified-v3b/iphone-cold-S2-large.png`; `iphone-cold-S2-large-readback.log` = `large` | `16,64 112x41` / `16,117 370x44` / `16,192 152x24` | PENDING/INSPECT |
| iphone S2 AX3 | `iphone-true-AX3-S2.txt`, `iphone-true-AX3-S2.png`; `iphone-true-AX3-readback.log` = `accessibility-extra-large` | `pending` / `16,117 370x96` / `16,244 311x51` | `after-verified-v3b/iphone-cold-S2-accessibility-extra-large-ui.txt`, `after-verified-v3b/iphone-cold-S2-accessibility-extra-large.png`; `iphone-cold-S2-accessibility-extra-large-readback.log` = `accessibility-extra-large` | `16,61 124x46` / `16,117 370x96` / `16,244 311x51` | PENDING/INSPECT |
| iphone S2 AX5 | `iphone-true-AX5-S2.txt`, `iphone-true-AX5-S2.png`; `iphone-true-AX5-readback.log` = `accessibility-extra-extra-extra-large` | `pending` / `16,117 370x124` / `16,272 239x131` | `after-verified-v3b/iphone-cold-S2-AX5-ui.txt`, `after-verified-v3b/iphone-cold-S2-AX5.png`; `iphone-cold-S2-AX5-readback.log` = `accessibility-extra-extra-extra-large` | `16,61 124x46` / `16,117 370x124` / `16,600 370x125` | VALID (fixed two-line row) |
| iphone S3 large | `iphone-large-S3.txt`, `iphone-large-S3.png`; `iphone-content-size-final-readback.log` = `large` | `16,64 94x41` / `pending` / `pending` | `after-verified-v3b/iphone-cold-S3-large-ui.txt`, `after-verified-v3b/iphone-cold-S3-large.png`; `iphone-cold-S3-large-readback.log` = `large` | `16,64 112x41` / `16,117 370x44` / `16,192 152x24` | PENDING/INSPECT |
| iphone S3 AX3 | `iphone-true-AX3-S3.txt`, `iphone-true-AX3-S3.png`; `iphone-true-AX3-readback.log` = `accessibility-extra-large` | `pending` / `16,117 370x96` / `16,244 311x51` | `after-verified-v3b/iphone-cold-S3-accessibility-extra-large-ui.txt`, `after-verified-v3b/iphone-cold-S3-accessibility-extra-large.png`; `iphone-cold-S3-accessibility-extra-large-readback.log` = `accessibility-extra-large` | `16,61 124x46` / `16,117 370x96` / `16,244 311x51` | PENDING/INSPECT |
| iphone S3 AX5 | `iphone-true-AX5-S3.txt`, `iphone-true-AX5-S3.png`; `iphone-true-AX5-readback.log` = `accessibility-extra-extra-extra-large` | `pending` / `16,117 370x124` / `16,272 239x131` | `after-verified-v3b/iphone-cold-S3-accessibility-extra-extra-extra-large-ui.txt`, `after-verified-v3b/iphone-cold-S3-accessibility-extra-extra-extra-large.png`; `iphone-cold-S3-accessibility-extra-extra-extra-large-readback.log` = `accessibility-extra-extra-extra-large` | `16,61 124x46` / `16,117 370x124` / `16,272 239x131` | PENDING/INSPECT |
| ipad S1 large | `ipad-search-S1.txt`, `ipad-search-S1.png`; `ipad-content-size-final-readback.log` = `large` | `pending` / `20,87 780x44` / `pending` | `after-verified-v3b/ipad-cold-S1-large-ui.txt`, `after-verified-v3b/ipad-cold-S1-large.png`; `ipad-cold-S1-large-readback.log` = `large` | `pending` / `20,87 780x44` / `pending` | PENDING/INSPECT |
| ipad S1 AX3 | `ipad-true-AX3-S1.txt`, `ipad-true-AX3-S1.png`; `ipad-true-AX3-readback.log` = `accessibility-extra-large` | `pending` / `20,87 780x96` / `pending` | `after-verified-v3b/ipad-cold-S1-accessibility-extra-large-ui.txt`, `after-verified-v3b/ipad-cold-S1-accessibility-extra-large.png`; `ipad-cold-S1-accessibility-extra-large-readback.log` = `accessibility-extra-large` | `pending` / `20,87 780x96` / `pending` | PENDING/INSPECT |
| ipad S1 AX5 | `ipad-true-AX5-S1.txt`, `ipad-true-AX5-S1.png`; `ipad-true-AX5-readback.log` = `accessibility-extra-extra-extra-large` | `pending` / `20,87 780x124` / `pending` | `after-verified-v3b/ipad-cold-S1-accessibility-extra-extra-extra-large-ui.txt`, `after-verified-v3b/ipad-cold-S1-accessibility-extra-extra-extra-large.png`; `ipad-cold-S1-accessibility-extra-extra-extra-large-readback.log` = `accessibility-extra-extra-extra-large` | `pending` / `20,87 780x124` / `pending` | PENDING/INSPECT |
| ipad S2 large | `ipad-search-S2.txt`, `ipad-search-S2.png`; `ipad-content-size-final-readback.log` = `large` | `pending` / `20,87 780x44` / `16,162 152x24` | `after-verified-v3b/ipad-cold-S2-large-ui.txt`, `after-verified-v3b/ipad-cold-S2-large.png`; `ipad-cold-S2-large-readback.log` = `large` | `pending` / `20,87 780x44` / `16,162 152x24` | PENDING/INSPECT |
| ipad S2 AX3 | `ipad-true-AX3-S2.txt`, `ipad-true-AX3-S2.png`; `ipad-true-AX3-readback.log` = `accessibility-extra-large` | `pending` / `20,87 780x96` / `16,214 311x52` | `after-verified-v3b/ipad-cold-S2-AX3-repair-ui.txt`, `after-verified-v3b/ipad-cold-S2-AX3-repair.png`; `ipad-cold-S2-AX3-repair-readback.log` = `accessibility-extra-large` | `pending` / `20,87 780x96` / `16,214 311x52` | REPAIRED VALID (replaced wrong-surface capture) |
| ipad S2 AX5 | `ipad-true-AX5-S2.txt`, `ipad-true-AX5-S2.png`; `ipad-true-AX5-readback.log` = `accessibility-extra-extra-extra-large` | `pending` / `20,87 780x124` / `16,242 394x66` | `after-verified-v3b/ipad-cold-S2-accessibility-extra-extra-extra-large-ui.txt`, `after-verified-v3b/ipad-cold-S2-accessibility-extra-extra-extra-large.png`; `ipad-cold-S2-accessibility-extra-extra-extra-large-readback.log` = `accessibility-extra-extra-extra-large` | `pending` / `pending` / `pending` | PENDING/INSPECT |
| ipad S3 large | `ipad-search-S3.txt`, `ipad-search-S3.png`; `ipad-content-size-final-readback.log` = `large` | `pending` / `20,87 780x44` / `16,162 152x24` | `after-verified-v3b/ipad-cold-S3-large-ui.txt`, `after-verified-v3b/ipad-cold-S3-large.png`; `ipad-cold-S3-large-readback.log` = `large` | `pending` / `20,87 780x44` / `16,162 152x24` | PENDING/INSPECT |
| ipad S3 AX3 | `ipad-true-AX3-S3.txt`, `ipad-true-AX3-S3.png`; `ipad-true-AX3-readback.log` = `accessibility-extra-large` | `pending` / `20,87 780x96` / `16,214 311x52` | `after-verified-v3b/ipad-cold-S3-accessibility-extra-large-ui.txt`, `after-verified-v3b/ipad-cold-S3-accessibility-extra-large.png`; `ipad-cold-S3-accessibility-extra-large-readback.log` = `accessibility-extra-large` | `pending` / `pending` / `pending` | PENDING/INSPECT |
| ipad S3 AX5 | `ipad-true-AX5-S3.txt`, `ipad-true-AX5-S3.png`; `ipad-true-AX5-readback.log` = `accessibility-extra-extra-extra-large` | `pending` / `20,87 780x124` / `16,242 394x66` | `after-verified-v3b/ipad-cold-S3-accessibility-extra-extra-extra-large-ui.txt`, `after-verified-v3b/ipad-cold-S3-accessibility-extra-extra-extra-large.png`; `ipad-cold-S3-accessibility-extra-extra-extra-large-readback.log` = `accessibility-extra-extra-extra-large` | `pending` / `pending` / `pending` | PENDING/INSPECT |
...
| Watched out / AX5 | `watched-AX5-out-ui.txt`, `watched-AX5-out-screen.png`, `watched-AX5-out-readback.log` | no valid AFTER out capture | pending |
| Watched in / `.large` | `watched-large-in-ui.txt`, `watched-large-in-screen.png`, `watched-large-in-readback.log` | `after/watched-parity-after-large.png`, `after/watched-parity-after-large-readback.log`, `after/watched-parity-after-emptysearch-ui.txt` | pending matched route/state; parity candidate only |
| Watched in / AX5 | `watched-AX5-in-ui.txt`, `watched-AX5-in-screen.png`, `watched-AX5-in-readback.log` | `after/watched-parity-after-AX5.png`, `after/watched-parity-after-AX5-readback.log`, `after/watched-parity-after-watched-ui.txt` | pending matched route/state; parity candidate only |
...
The W-10 root runtime used the same logger as the W-35 timing run; the complete source log is retained at `$HOME/Library/Caches/ehpanda-phase16/round2/walkthrough/vo3/w10/root-runtime/phase16-w35-timing-root.log`, with the bounded timeline at `timeline.txt` and the PNGs in that directory. The malformed-URL error toast was announced at `17:19:36.122`. A forward walk reached actual/spoken `Error` at `17:19:48.277` / `.279` after five tabs, and reached the preceding Setting at `17:20:11.657` / `.658`; these are bounded observations rather than the full Task 7 walk. The Filters control was actual/spoken at `17:20:37.173` / `.188`; after the sheet Cancel at `17:25:36`, Search was actual/spoken at `17:25:37.425` / `.445`. The known Fixture history item was actual/spoken at `17:26:48.093` / `.094`, results were captured in `phase16-w10-root-results.png`, Back was actual/spoken at `17:27:44.808` / `.809`, and popping to Search was actual/spoken at `17:28:00.418` / `.434`. The final Search PNG still shows the error toast, confirming this bounded route. The 55-case module gate and generic Simulator build remain passing; Task 7's complete walk2/UI batch remains pending.
