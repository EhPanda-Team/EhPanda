...
key-decisions:
  ...
  - "[Corrected by 15-74] Sibling composition is a CONVENTION verified by inspection of the four callers at this HEAD, not a property the language enforces: the bracket's closure is non-escaping and non-Sendable, so it inherits actor isolation and a synchronous advance written inside a bracket body compiles. 15-74 added the depth counter that detects a nest"
...
- **[Corrected by 15-74] Sibling composition is a four-item INSPECTION, and the per-site audit below is the whole of the evidence.** This bullet claimed the opposite: that because `withdrawingCountedBasisMovement` takes a non-async, non-escaping `() throws -> T` while all four callers of the advance are `async` functions, no caller could appear lexically inside a bracket closure. That inference does not hold. The closure is non-escaping and non-`Sendable`, so it inherits the enclosing actor isolation, and `advanceQueueIntentGeneration(for:)` is itself a *synchronous* actor-isolated method — writing it inside a bracket body compiles today and withdraws its delta twice. The four dispositions below are true and were re-derived independently by the round-19 verifier; what was false was calling them redundant. 15-74 replaced the claim with a detector: the bracket keeps a depth counter and reports an issue when one opens inside another.
...
**[Corrected by 15-74] There is no stronger, construction-level proof — the table above is the evidence.** This paragraph argued that because `withdrawingCountedBasisMovement<T>(gid:_:)` takes a non-async, non-escaping `() throws -> T` while all four callers are `async` (`performRetry(...) async throws`, `performRetryPages(...) async throws`, `resume(gid:) async`, `enqueue(payload:) async`), a nest would not compile. It would. The closure's non-escaping, non-`Sendable` shape makes it inherit the enclosing actor isolation, so any synchronous actor-isolated method is callable inside it — and the advance is exactly such a method, which means the "future *synchronous* caller" this paragraph named as the one dangerous shape is the very method being discussed. The four call sites really are siblings at that HEAD; that is an inspection result and nothing more. 15-74 made the breach detectable instead of merely forbidden: the bracket increments a depth counter around the movement and reports an issue (never a trap) when a second bracket opens inside the first, pinned by a probe that deliberately writes the forbidden shape.
