...
|----------|-------------|---------------|
| app process → `BGTaskScheduler` daemon | The permitted-identifier declaration is the app's contract with the system scheduler; registration and submission cross here. Registration crosses at most once per process: one identifier (bundle id + `continued` + UUID) is minted on the first start and reused by every later session's submission, which is what the never-unregistered-handler row below depends on. | Task identifier (one per process), card title, card subtitle |
...
| T-15-03 (15-01/03/07) | Elevation of Privilege | `App/Info.plist` `BGTaskSchedulerPermittedIdentifiers` | medium | mitigate | Exactly one entry, `$(PRODUCT_BUNDLE_IDENTIFIER).continued.*` — bundle-prefixed plus one semantic segment, never a bare wildcard. The one minted identifier is runtime bundle id + `.continued.` + UUID; no wildcard is ever passed to `register`. `App/Info.plist`, `ContinuedProcessingSession.swift:165`. | closed |
...
| T-15-09 (15-03/05) | Denial of Service (app termination) | Scheduler registration | high | mitigate | `register` is called at most once per process: the first start mints one identifier, stores it in `registeredIdentifier`, and every later session reuses it for submission alone, so the double-registration the system punishes with termination is structurally unreachable (15-51 superseded the earlier fresh-UUID-per-session rule, whose per-session registrations were each a fresh exposure). The UUID suffix keeps the one identifier distinct from any a previous build registered. The store refuses re-entry before any scheduler touch (`guard task == nil, continuation == nil, !isAwaitingTask`), and the coordinator sets `hasLiveContinuedSession` synchronously before its first suspension point. `ContinuedProcessingSession.swift:117-121, 156-180`; `DownloadClient+ContinuedSession.swift`, `ensureContinuedSession`. | closed |
