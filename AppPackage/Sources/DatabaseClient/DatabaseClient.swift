...
public struct DatabaseClient: Sendable {
    ...
    public let dropDatabase: @Sendable () async -> Result<Void, AppError>
...
extension DatabaseClient {
    ...
    public static func live(persistenceContainer container: NSPersistentContainer) -> Self {
        .init(
...
// MARK: Foundation
extension DatabaseClient {
    public func batchFetch<MO: NSManagedObject>(
    ...
    public func fetch<MO: NSManagedObject>(
    ...
    public func fetchOrCreate<MO: NSManagedObject>(
        entityType: MO.Type, predicate: NSPredicate? = nil,
    ...
    public func update<MO: NSManagedObject>(
        entityType: MO.Type, predicate: NSPredicate? = nil,
...
extension DatabaseClient {
    public func fetch<MO: GalleryIdentifiable>(
    ...
    public func fetchOrCreate<MO: GalleryIdentifiable>(entityType: MO.Type, gid: String) -> MO {
        fetchOrCreate(
            entityType: entityType,
    ...
    public func update<MO: GalleryIdentifiable>(
        entityType: MO.Type, gid: String,
...
extension DatabaseClient {
    public func update<T: Codable>(gid: String, storedData: inout Data?, new: [Int: T]) {
...
// MARK: Fetch
extension DatabaseClient {
    public func fetchGallery(gid: String) -> Gallery? {
    ...
    public func fetchGalleryDetail(gid: String) -> GalleryDetail? {
        guard gid.isValidGID else { return nil }
    ...
    @MainActor public func fetchAppEnv() -> AppEnv {
        fetchOrCreate(entityType: AppEnvMO.self).toEntity()
    }
    public func fetchAppEnvSynchronously() -> AppEnv {
        fetchOrCreate(entityType: AppEnvMO.self).toEntity()
    }
    @MainActor public func fetchGalleryState(gid: String) async -> GalleryState? {
        guard gid.isValidGID else { return nil }
        return fetchOrCreate(entityType: GalleryStateMO.self, gid: gid).toEntity()
    }
    @MainActor public func fetchHistoryGalleries(fetchLimit: Int = 0) -> [Gallery] {
...
// MARK: FetchAccessor
extension DatabaseClient {
    public func fetchFilterSynchronously(range: FilterRange) -> Filter {
        switch range {
        case .search:
    ...
    @MainActor public func fetchHistoryKeywords() -> [String] {
        fetchAppEnv().historyKeywords
    }
    @MainActor public func fetchQuickSearchWords() -> [QuickSearchWord] {
        fetchAppEnv().quickSearchWords
    }
    @MainActor public func fetchGalleryPreviewURLs(gid: String) async -> [Int: URL]? {
        guard gid.isValidGID else { return nil }
...
// MARK: UpdateGallery
extension DatabaseClient {
    @MainActor public func updateGallery(gid: String, key: String, value: Any?) {
    ...
    @MainActor public func updateLastOpenDate(gid: String, date: Date = .now) {
        guard gid.isValidGID else { return }
        updateGallery(gid: gid, key: "lastOpenDate", value: date)
    }
    @MainActor public func clearHistoryGalleries() {
        let predicate = NSPredicate(format: "lastOpenDate != nil")
    ...
    @MainActor public func cacheGalleries(_ galleries: [Gallery]) {
...
extension DatabaseClient {
    @MainActor public func cacheGalleryDetail(_ detail: GalleryDetail) {
...
public enum DatabaseClientKey: DependencyKey {
    ...
    public static let previewValue = DatabaseClient.noop
    public static let testValue = DatabaseClient.unimplemented
}
