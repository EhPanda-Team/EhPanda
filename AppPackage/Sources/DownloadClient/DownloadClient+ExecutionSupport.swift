...
extension DownloadCoordinator {
    ...
    /// mover that converges here — the folder deletion, the fresh manifest, the blanking, the
    /// post-removal recovery, and any sibling a later round adds — is enclosed by construction
    /// rather than instrumented one at a time.
    ///
    /// **Every post-removal exit compensates, and the table is here so the family cannot be handled
    /// piecemeal again (WR-03).** `authorizedReconciliationScan` destroys page files before this
    /// function writes anything, deliberately — the reverse order would leave a failed removal as a
    /// blank hash beside a surviving file, the D-SSOT-04 laundering shape. Its price is a window with
    /// exactly three exits, and 15-67 shipped the ordering with none of them handled:
    ///
    /// 1. **The post-removal rescan could not enumerate**, so
    ///    `reconcileWorkingManifestAgainstPageFiles` returns the manifest VERBATIM at its
    ///    `scanSucceeded` guard. *Disposition: recover once, then proceed* — the run route is
    ///    self-healing here, because the removed pages are absent from `existingPages` and therefore
    ///    pending, so `pendingPageIndices` re-fetches exactly them. Pinned by
    ///    `testASeedRescanFailureCannotAnnounceMoreThanTheHonestBasis`.
    /// 2. **The post-removal loop's OWN refusal lines fire** — a page the rescan demoted to a
    ///    non-answer, or a file that vanished in the race, growing the wholesale term past the
    ///    threshold the pre-removal guard measured. *Disposition: recover once, then proceed* — the
    ///    state is identical to exit 1's and self-heals the same way. It shares exit 1's case
    ///    deliberately: both are decided here by the ONE predicate `claimsAnyPage`, because what the
    ///    record shows is the loop's answer and not its reason, and a disposition keyed on the reason
    ///    would be a second rule to keep in step.
    /// 3. **`storage.writeManifest` threw out of the loop**, so the removal stands against an
    ///    unchanged record. *Disposition: recover once, then PROPAGATE* — this preparation could not
    ///    persist what it decided, so the run must fail; what it may not do is fail over a lying
    ///    record. Pinned by `testAThrownSeedManifestWriteRecoversTheRecordBeforeFailing`.
    ///
    /// All three reach `recoveredBlanking`, the validate route's own implementation, which is the
    /// point: a second copy of the ordering is what dropped the compensation, and a second copy of
    /// the recovery would drop it again. It opens no bracket, so it runs inside THIS one — the
    /// bracket rule is that movements compose as SIBLINGS and never nest.
    ///
    /// **Exit 3 returns a `Result` rather than throwing out of the bracket, and that is load-bearing
    /// rather than stylistic.** Its compensation can succeed, which LOWERS the record's counted
    /// basis; a throw would unwind past the bracket's second reading and withdraw nothing, leaving
    /// the monotonic floor holding a count the record no longer supports — G-15-7's masking shape,
    /// reintroduced through the recovery. The pre-removal steps still throw straight through,
    ...
    private func prepareWorkingSeed(
        ...
        let outcome: Result<WorkingSeed, any Error> = try withdrawingCountedBasisMovement(
            ...
            var reconciledManifest: DownloadManifest
            do {
                reconciledManifest = try reconcileWorkingManifestAgainstPageFiles(
                    manifest: manifest,
                    pageFileScan: reconciliationScan,
                    folderURL: folderURL
                )
            } catch {
                // EXIT 3. Compensate, then propagate. The guard is not an optimisation: where this
                // preparation destroyed nothing the record still claims exactly what it claimed
                // before, so there is no divergence to recover and no removal to log.
            ...
            if reconciledManifest.claimsAnyPage(in: removedPages) {
                // EXITS 1 and 2. The loop answered with a record that still claims a page whose file
                // this preparation deleted. A successful retry's manifest is carried forward rather
                // than discarded: the seed's record is what the announcement and the fetch filter
                // read, so keeping the stale copy would announce against claims just retracted.
                reconciledManifest = recoveredBlanking(
                    gid: payload.gallery.gid,
                    manifest: manifest,
                    removedPages: removedPages,
                    folderURL: folderURL
            ...
            return .success(
                .init(
                    ...
                    // consumed — every one of these three members, so the credit rule and the
                    // blanking rule can never answer from different probes. That is why they follow
                    // the reconciliation scan across an authorized removal rather than staying
                    // pinned to the pre-removal classification: a page whose refuted file this
                    // preparation just deleted is not a page the run may treat as already present.
    ...
    /// and file-private because only `prepareWorkingSeed` consumes it — what crosses out of that
    /// function is `WorkingSeed.removedPages`.
    private struct AuthorizedReconciliation {
    ...
    public func pendingPageIndices(
        ...
        // set admits exactly its members, INCLUDING the empty set, which admits nothing. A guard
        // written as "non-empty selection restricts, everything else does not" would re-open
        // exactly the widening CR-04 closed, so the empty-present branch stays.
        //
        // WR-05: that branch is now DEFENCE IN DEPTH rather than a live state.
        // `normalizeFetchedPayload` throws when the freshly fetched page count leaves none of an
        // explicit selection standing, so no payload carrying an empty selection reaches this
        // function any more — the run fails at that boundary with a reason instead of arriving here
        // to schedule nothing and then be measured against the whole manifest at finalize. The
        // branch is kept because the reading is what makes the presence meaningful: a future
        // producer of `payload.pageSelection` inherits the correct answer rather than the widening.
